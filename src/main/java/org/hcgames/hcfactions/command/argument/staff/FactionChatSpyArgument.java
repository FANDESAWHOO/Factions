...
public final class FactionChatSpyArgument{ //Command temp disabled || WHY SYSTEM UPDATE? WHY YOU DON'T LET COMMENTS!!!!!
//public class FactionChatSpyArgument extends CommandArgument implements Listener {
    ...
    public FactionChatSpyArgument(HCFactions plugin) {
    ...
    public void onFactionChat(FactionChatEvent event) {
        ...
        Collection<CommandSender> recipients = new HashSet<>(HCF.getOnlinePlayers());
    ...
    public List<String> onTabComplete(CommandSender sender, Command command, String label, String[] args) {
        if (args.length == 2) {
            ...
            results.addAll(HCF.getOnlinePlayers().stream().filter(player -> senderPlayer == null || senderPlayer.canSee(player)).map(Player::getName).collect(Collectors.toList()));
            return results;
        } else {
