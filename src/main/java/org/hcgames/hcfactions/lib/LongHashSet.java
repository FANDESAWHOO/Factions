...
public class LongHashSet {
	...
	public boolean add(long value) {
		...
		while (values[index] != FREE && !(hash(values[index]) == hash && values[index] == value)) {
			...
			if (values[index] == REMOVED) deletedix = index;
		...
		if (values[index] == FREE) {
			...
			if (1 - (freeEntries / (double) values.length) > LOAD_FACTOR) rehash();
	...
	public void clear() {
		elements = 0;
		for (int ix = 0; ix < values.length; ix++) values[ix] = FREE;
	...
	public long[] toArray() {
		...
		for (long value : values) if (value != FREE && value != REMOVED) result[pos++] = value;
	...
	private void rehash() {
		...
		if (gargagecells / (double) values.length > 0.05) rehash(values.length);
		else rehash(values.length * 2 + 1);
	}
	...
	private void rehash(int newCapacity) {
		...
		for (long value : values) {
			if (value == FREE || value == REMOVED) continue;
	...
	private class Itr implements Iterator {
		...
		public Long next() {
			...
			if (values[lastReturned] == FREE) return FREE;
			else return values[lastReturned];
		}
