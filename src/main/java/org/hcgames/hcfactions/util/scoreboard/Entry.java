...
package org.hcgames.hcfactions.util.scoreboard;
...
class Entry {
	...
	void setValue(String strValue) {
		...
		Team newTeam = board.getHandle().registerNewTeam(newValue.getName());
	...
	static class Value {
		...
		static Value split(@NonNull String display) {
			...
			result[resultIndex] = currentBuilder.toString();
			if (result[2] == null)
				result[2] = "";//TODO: Implement, push as much as possible into the suffix from the name. If name length == suffix length && suffix is empty then put suffix as name and name as unique key
