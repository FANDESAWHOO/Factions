...
public abstract class PlayerTimer extends Timer{
    ...
    public void clearAllCooldowns(){
        for(UUID uuid : ImmutableSet.copyOf(cooldowns.keySet())) setCooldown(null, uuid);
    }
    ...
    public boolean setCooldown(@Nullable Player player, UUID playerUUID, long duration, boolean overwrite, @Nullable Predicate<Long> currentCooldownPredicate){
        ...
        if(runnable != null){
            ...
            boolean flag = true;
            if(currentCooldownPredicate != null) flag = currentCooldownPredicate.test(remaining);
    ...
    public void load(Config config){
        ...
        if(object instanceof MemorySection){
            ...
            for(String id : section.getKeys(false)){
                ...
                if(remaining > 0L) setCooldown(null, UUID.fromString(id), remaining, true, null);
            }
        }
